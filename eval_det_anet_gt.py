...
def add_class(proposals, class_name):
    detections = []
    for proposal in proposals:
        detection = {'segment': proposal['segment']}
        detection['score'] = proposal['score']
...
def gen_detection(prop_file, cls_file, out_file):
    ...
    for video_name, results in proposals['results'].items():
        class_name = classifications[video_name]
        detections['results'][video_name] = add_class(results, class_name)
